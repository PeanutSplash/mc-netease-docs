...
*   参数
    ...
    （上下角度，左右角度）实体水平方向的俯仰角度和竖直方向的旋转角度，单位是角度而不是弧度。[MC坐标系说明](../../../../mcguide/20-玩法开发/10-基本概念/10-Vector3.html)
...
*   备注
    ...
    *   回调函数传入的参数dict
        ...
        详见[EntityType枚举](/dev/mcmanual/mc-dev/mcdocs/1-ModAPI-beta/枚举值/EntityType.html)
...
*   参数
    ...
    [RayFilterType枚举](/dev/mcmanual/mc-dev/mcdocs/1-ModAPI-beta/枚举值/RayFilterType.html)（选填，默认为serverApi.GetMinecraftEnum().RayFilterType.OnlyEntities）
...
*   备注
    ...
    *   没有选中目标时，返回值为：None
    *   假设方块的clip(计算射线检测时用的碰撞盒)没有体积时，射线检测将会无视这个方块，例如原版的火焰，灵魂火等；自定义方块的clip可以在[netease:aabb](../../../../mcguide/20-玩法开发/15-自定义游戏内容/2-自定义方块/1-JSON组件.html#netease-aabb)中进行设置
*   示例
