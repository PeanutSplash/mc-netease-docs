...
*   备注
    ...
    *   该接口不屏蔽玩家控制的移动以及重力作用，当有玩家控制发生时，最终的表现结果可能与预期有差异。由于玩家的头部与相机控制相关，若需要使运动器控制玩家的头部，请使用[DepartCamera](/接口/玩家/摄像机)分离玩家与摄像机。
...
*   备注
    ...
    *   该接口不屏蔽玩家控制的移动，当有玩家控制发生时，最终的表现结果可能与预期有差异。由于玩家的头部与相机控制相关，若需要使运动器控制玩家的头部，请使用[DepartCamera](/接口/玩家/摄像机)分离玩家与摄像机。
    *   轨迹运动器不可叠加，仅能添加一个。
...
*   参数
    ...
    行为枚举[PlayerExhauseRatioType枚举](/枚举值/PlayerExhauseRatioType)
...
*   备注
    ...
    *   1.会触发[PlayerAttackEntityEvent](/事件/玩家)事件，并且可以被这个事件cancel
    *   2.此接口无视距离，但无法跨维度使用，同时需要目标区域已加载
    *   3.常加载区块的生物也不会执行tick，生物受伤cd不会减少，无法对生物多次造成伤害，通过[SetHurtCD](/接口/世界/游戏规则)接口设置伤害间隔为0，可以解决这个问题
    *   4.此接口不会播放原版攻击动作，可以使用[Swing](/接口/玩家/属性)接口来播放原版攻击动作
*   示例
...
*   备注
    ...
    *   1.会触发[PlayerInteractServerEvent](/事件/玩家)事件，并且可以被这个事件cancel
    *   2.使用PlayerUseItemToEntity接口模拟玩家用命名牌重命名生物、用食物喂养生物时，可以触发[PlayerNamedEntityServerEvent](/事件/玩家)、[PlayerFeedEntityServerEvent](/事件/玩家)事件，并且这两个事件都可以cancel
...
*   参数
    ...
    物品所在的地方[ItemPosType枚举](/枚举值/ItemPosType)
...
*   备注
    ...
    *   该接口只修改服务端的交互中心，所以需要使用[SetCameraAnchor](/接口/玩家/摄像机)修改客户端第一人称的交互中心，使用[SetPickCenterOffset](https://mc.163.com/dev/mcmanual/mc-dev/mcdocs/1-ModAPI-beta/%E6%8E%A5%E5%8F%A3/%E7%8E%A9%E5%AE%B6/#setpickcenteroffset)修改客户端第三人称的交互中心，否则客户端将无法选中方块，即无法发送正确的信息给服务端
    *   注：当前客户端能修改交互中心的有[SetCameraAnchor](/接口/玩家/摄像机)修改第一人称，[SetPickCenterOffset](https://mc.163.com/dev/mcmanual/mc-dev/mcdocs/1-ModAPI-beta/%E6%8E%A5%E5%8F%A3/%E7%8E%A9%E5%AE%B6/#setpickcenteroffset)修改第三人称。服务端能使用[SetInteracteCenterOffset](https://mc.163.com/dev/mcmanual/mc-dev/mcdocs/1-ModAPI-beta/%E6%8E%A5%E5%8F%A3/%E7%8E%A9%E5%AE%B6/#setinteractecenteroffset)修改玩家交互中心（服务端无第几人称概念）
*   示例
...
*   备注
    ...
    *   该接口只修改第三人称，第一人称下需要使用[SetCameraAnchor](/接口/玩家/摄像机)接口设置交互中心。
...
*   备注
    ...
    *   该接口影响接口[SetHurtCD](/接口/世界/游戏规则)设置的全局受击间隔CD
*   示例
...
*   备注
    ...
    *   由于玩家的运动器需要在客户端与服务端之间同步，所以请以[EntityMotionStartServerEvent](/事件/实体)事件的触发作为真正的开始时机。
*   示例
...
*   备注
    ...
    *   调用该接口不会触发事件[EntityMotionStopServerEvent](/事件/实体)。
*   示例
