...
*   参数
    ...
    | targetRot | tuple(float,float,float) | 相机到达targetPos时的朝向，受参数relativeCoord影响，默认为None，表示使用调用StartCameraMotion时的朝向。 |
    | startRot | tuple(float,float,float) | 相机到达startPos时的朝向，受参数relativeCoord影响，默认为None，表示使用调用StartCameraMotion时的朝向。 |
