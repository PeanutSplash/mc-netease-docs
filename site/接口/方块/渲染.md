---
title: "渲染"
source_url: "https://mc.163.com/dev/mcmanual/mc-dev/mcdocs/1-ModAPI/%E6%8E%A5%E5%8F%A3/%E6%96%B9%E5%9D%97/%E6%B8%B2%E6%9F%93.html"
...
*   参数
    ...
    | 参数名 | 数据类型 | 说明 |
    | --- | --- | --- |
    | entityId | str | 掉落物的entityId，entityId可在调用[AddDropItemToWorld](/接口/方块/渲染#AddDropItemToWorld)时保存返回值，或者通过[GetClientDropItemEntityIdList](/接口/方块/渲染#GetClientDropItemEntityIdList)接口获取list |
...
*   参数
    ...
    | face | int | 需要获取的方块面，参考[Facing枚举](/枚举值/Facing)，face默认值为6，此时获取方块所有面的贴图信息 |
...
*   参数
    ...
    | rotation | tuple(float,float,float) | 选填，设置掉落物的角度，不填默认为None，设置rotation后将会固定角度[AddDropItemToWorld](/接口/方块/渲染#AddDropItemToWorld)设置的spinSpeed无效。如需设置后续参数需填写None占位，传None表示不修改 |
