---
title: "微软粒子"
source_url: "https://mc.163.com/dev/mcmanual/mc-dev/mcdocs/1-ModAPI-beta/%E6%8E%A5%E5%8F%A3/%E7%89%B9%E6%95%88/%E5%BE%AE%E8%BD%AF%E7%B2%92%E5%AD%90.html?catalog=1"
scraped_at: "2025-09-13T09:58:44.487Z"
...
*   参数
    ...
    需要绑定的骨骼名称(不区分大小写) 默认值为"body"，传入空字符串时代表根骨骼。包含多个根骨骼的情况下，选择按骨骼名称排序后的首个根骨骼。
    
    offset
    
    tuple(float,float,float)
    
    三维 表示粒子发射器的绑定偏移 默认值为(0, 0, 0)
    
    rotation
    
    tuple(float,float,float)
    
    表示粒子发射器绑定的三维旋转(角度制，按照ZYX顺序旋转) 默认值为(0, 0, 0)
...
*   参数
    ...
    绑定的骨骼模型的ID（见model组件的GetModelId）
    
    bone\_name
    
    str
    
    绑定具体骨骼的名称(不区分大小写)，默认值为"root"
...
*   参数
    ...
    表示粒子发射器绑定的三维旋转(使用角度制，按照ZYX顺序旋转) 默认值为(0, 0, 0)
...
*   返回值
    ...
    表示粒子发射器在当前播放周期内已播放的时间 粒子发射器id无效时会返回0.0
...
*   参数
    ...
    表示是否获取局部空间旋转 默认值为True(即 默认获取局部空间旋转)
...
*   参数
    ...
    Molang变量名(例如variable.emitter\_age)
...
*   参数
    ...
    粒子发射器id 需要粒子发射器已绑定实体
    
    is\_relative
    
    bool
    
    表示粒子是否在局部空间进行计算 默认参数值为True
...
*   参数
    ...
    粒子发射器三维旋转(使用角度制，按照ZYX顺序旋转) 默认值为(0, 0, 0)
...
*   参数
    ...
    粒子发射器三维旋转(使用角度制，按照ZXY顺序旋转) 默认值为(0, 0, 0)
...
*   参数
    ...
    解绑后是否保持粒子发射器在世界空间的位置 默认值为True
    
    keep\_rotation
    
    bool
    
    解绑后是否保持粒子发射器在世界空间的旋转 默认值为True
