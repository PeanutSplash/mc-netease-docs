---
title: "序列帧"
source_url: "https://mc.163.com/dev/mcmanual/mc-dev/mcdocs/1-ModAPI-beta/%E6%8E%A5%E5%8F%A3/%E7%89%B9%E6%95%88/%E5%BA%8F%E5%88%97%E5%B8%A7.html?catalog=1"
...
*   参数
    ...
    创建位置，可选，没传则可以创建完用frameAniTrans组件设置
    
    rot
    
    tuple(float,float,float)
    
    角度，可选，没传则可以创建完用frameAniTrans组件设置
    
    scale
    
    tuple(float,float,float)
    
    缩放系数，可选，没传则可以创建完用frameAniTrans组件设置
...
*   参数
    ...
    创建位置，可选，没传则可以创建完用frameAniTrans组件设置，一般需要设置播放的位置
...
*   参数
    ...
    对于平面序列帧，第一个参数为贴图横向上的缩放，第二个参数为纵向上的缩放，第三个参数无用。对于环状序列帧，为三个坐标轴上的缩放
