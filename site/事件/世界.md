---
title: "世界"
source_url: "https://mc.163.com/dev/mcmanual/mc-dev/mcdocs/1-ModAPI/%E4%BA%8B%E4%BB%B6/%E4%B8%96%E7%95%8C.html"
...
*   备注
    ...
    *   该事件触发只表明在服务端数据中接收到了新玩家，并不能代表此时玩家在客户端中可见，若想在玩家进入视野后立马调用玩家渲染相关接口，建议使用[AddPlayerCreatedClientEvent](/事件/世界#addplayercreatedclientevent)。
...
*   备注
    ...
    *   由于玩家皮肤是异步加载的原因，该事件触发时机比[AddPlayerAOIClientEvent](/事件/世界#addplayeraoiclientevent)晚，触发该事件后可以对该玩家调用相关[玩家渲染接口](/接口/玩家/渲染)。
...
*   备注
    ...
    *   对于玩家传入的参数，type不同则对应的value也不同，type对应value的类型和具体数据格式如下
        ...
        | effect | dict | 含有状态效果名称和对应的[EffectType](/枚举值/EffectType)枚举，如{'name': 'absorption', 'id': 22} |
        | dimension | dict | 含有维度名称和对应数字id，如{'name': 'nether', 'id': 1} |
        | biome | dict | 含有生物群系名称和对应的[BiomeType](/枚举值/BiomeType)枚举，如{'name': 'bamboo_jungle', 'biomeType': 48} |
        | structure | dict | 含有结构名称和对应的[StructureFeatureType](/枚举值/StructureFeatureType)枚举，如{'name': 'ancient_city', 'structureType': 15} |
        | enchant | dict | 含有附魔名称和对应的[EnchantType](/枚举值/EnchantType)枚举，如{'identifier': 'knockback', 'type': 12} |
...
*   参数
    ...
    | entityType | int | 期望响应的实体类型，不传则响应所有的实体类型[EntityType枚举](/枚举值/EntityType) |
...
*   参数
    ...
    | itemList | list | 掉落物品列表，每个元素为一个itemDict，格式可参考[物品信息字典](https://mc.163.com/dev/mcmanual/mc-dev/mcguide/20-玩法开发/10-基本概念/1-我的世界基础概念.html#物品信息字典#物品信息字典) |
...
*   参数
    ...
    | type | int | 生成实体的类型，参考[EntityType](/枚举值/EntityType) |
