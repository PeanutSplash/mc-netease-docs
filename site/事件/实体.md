...
*   备注
    ...
    *   注意：该事件触发表示运动器播放顺利完成，手动调用的[StopEntityMotion](/接口/实体/行为)、[RemoveEntityMotion](/接口/实体/行为)以及实体被销毁导致的运动器停止不会触发该事件。
...
*   备注
    ...
    *   注意：不能在该事件回调中对此玩家手持物品进行修改，如[SpawnItemToPlayerCarried](/接口/玩家/背包)、[ChangePlayerItemTipsAndExtraId](/接口/玩家/背包)等接口
