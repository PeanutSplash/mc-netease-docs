...
触发时机：监听该事件的方块在销毁时触发，可以通过[ListenOnBlockRemoveEvent](#listenonblockremoveevent)方法进行监听，或者通过json组件[netease:listen\_block\_remove](../../../mcguide/20-玩法开发/15-自定义游戏内容/2-自定义方块/1-JSON组件.html#netease-listen-block-remove)进行配置
...
*   描述
    ...
    是否监听方块BlockRemoveServerEvent事件，可以动态修改json组件[netease:listen\_block\_remove](../../../mcguide/20-玩法开发/15-自定义游戏内容/2-自定义方块/1-JSON组件.html#netease-listen-block-remove)的值
...
*   参数
    ...
    方块进入脱离含雪的原因，参考[SetBlockType](/dev/mcmanual/mc-dev/mcdocs/1-ModAPI-beta/枚举值/SetBlockType.html)
...
*   参数
    ...
    方块被敲击的面向id，参考[Facing枚举](/dev/mcmanual/mc-dev/mcdocs/1-ModAPI-beta/枚举值/Facing.html)
...
*   备注
    ...
    *   不是所有下落的方块都会触发该事件，需要在json中先配置触发开关（详情参考：[自定义重力方块](../../../mcguide/20-玩法开发/15-自定义游戏内容/2-自定义方块/3-特殊方块/6-自定义重力方块.html)）
...
*   参数
    ...
    耕地退化为泥土的原因，参考[SetBlockType](/dev/mcmanual/mc-dev/mcdocs/1-ModAPI-beta/枚举值/SetBlockType.html)
...
*   备注
    ...
    *   只有添加了自定义方块实体扩展功能的自定义方块实体才能触发该事件（见[自定义方块实体外观](../../../mcguide/20-玩法开发/15-自定义游戏内容/2-自定义方块/4.1-自定义方块实体外观.html)），请用该事件来初始化自定义方块实体的molang，否则方块实体未初始化完成会设置失败。
...
*   备注
    ...
    *   只有client\_tick字段为true的自定义方块实体才能触发该事件（见[自定义方块实体](../../../mcguide/20-玩法开发/15-自定义游戏内容/2-自定义方块/4-自定义方块实体.html)
...
*   备注
    ...
    *   不是所有方块都会触发该事件，需要在json中先配置触发开关（详情参考：[自定义方块JSON组件](../../../mcguide/20-玩法开发/15-自定义游戏内容/2-自定义方块/1-JSON组件.html)）
...
*   备注
    ...
    *   不是所有方块都会触发该事件，需要在json中先配置触发开关（详情参考：[自定义方块JSON组件](../../../mcguide/20-玩法开发/15-自定义游戏内容/2-自定义方块/1-JSON组件.html)） ，原版方块需要先通过RegisterOnEntityInside接口注册才能触发
...
*   备注
    ...
    *   不是所有方块都会触发该事件，需要在json中先配置触发开关（详情参考：[自定义方块JSON组件](../../../mcguide/20-玩法开发/15-自定义游戏内容/2-自定义方块/1-JSON组件.html)） ，原版方块需要先通过RegisterOnStandOn接口注册才能触发
...
*   参数
    ...
    活塞的朝向，参考[Facing枚举](/dev/mcmanual/mc-dev/mcdocs/1-ModAPI-beta/枚举值/Facing.html)
    
    pistonMoveFacing
    
    int
    
    活塞的运动方向，参考[Facing枚举](/dev/mcmanual/mc-dev/mcdocs/1-ModAPI-beta/枚举值/Facing.html)
...
*   备注
    ...
    *   可通过[minecraft:destroy\_time](../../../mcguide/20-玩法开发/15-自定义游戏内容/2-自定义方块/1-JSON组件.html#minecraft_destroy_time)方块组件来修改方块的破坏时间
...
*   参数
    ...
    方块被敲击面,参考[Facing枚举](/dev/mcmanual/mc-dev/mcdocs/1-ModAPI-beta/枚举值/Facing.html)
...
*   备注
    ...
    *   秒破方块在本事件触发前已经被服务端删除，此时本事件获取到的blockName为minecraft:air，且无法通过本事件进行取消操作,以下是两个解决方法： （1）用ServerPlayerTryDestroyBlockEvent获取到正确的方块信息或取消操作。 （2）通过[minecraft:destroy\_time](../../../mcguide/20-玩法开发/15-自定义游戏内容/2-自定义方块/1-JSON组件.html#minecraft_destroy_time)方块组件来修改方块的破坏时间。
...
*   备注
    ...
    *   不是所有方块都会触发该事件，自定义方块需要在json中先配置触发开关（详情参考：[自定义方块JSON组件](../../../mcguide/20-玩法开发/15-自定义游戏内容/2-自定义方块/1-JSON组件.html)）， 原版方块需要先通过RegisterOnStepOff接口注册才能触发
    *   压力板与绊线钩这种非实心方块不会触发
...
*   备注
    ...
    *   不是所有方块都会触发该事件，自定义方块需要在json中先配置触发开关（详情参考：[自定义方块JSON组件](../../../mcguide/20-玩法开发/15-自定义游戏内容/2-自定义方块/1-JSON组件.html)）， 原版方块需要先通过RegisterOnStepOn接口注册才能触发。原版的红石矿默认注册了，但深层红石矿没有默认注册。
